...
## v0.15.2 — visites guidées à jour sur les 5 écrans

- **`lib/tour.ts` réécrit** pour couvrir les fonctionnalités arrivées depuis la création des
  visites : amines biogènes, satiété, échelle de Bristol, saisie vocale (repas & satiété),
  partage de fiche en carte PNG, référentiel d'aliments, dossier médical avec graphes.
- **Nouvelles étapes ancrées** (attributs `data-tour` posés dans les vues) :
  - Journal : `journal-analyze` (bouton « Analyser ma journée avec l'IA ») ; menu ⋯ et étape
    d'édition réécrits (satiété, transit, contexte stress/sommeil/règles).
  - Aliments : `aliments-scope` (De mes repas / Catalogue / Favoris), `aliments-scan`
    (code-barres), `aliments-actions` (idées de repas IA, analyse en masse, complément amines
    hors-ligne, export/import du référentiel).
  - Semaine : `week-stats` (cartes récap dont note amines /10 et score énergie),
    `week-correlations` (déclencheurs, aliments sûrs, contexte, amines — seuils honnêtes).
  - Évolution : `evo-report` (rapport de période) ; étape centrée sur les graphes conditionnels
    (symptômes après repas, aliments suspects, tendance amines, Bristol, satiété, synthèse IA).
  - Repères : `reperes-tabs` (Repères + fiche amines / Encyclopédie / Système digestif).
- Le repli en bulle centrée de `Tour.tsx` couvre les cibles absentes (rendu conditionnel) —
  aucune logique ajoutée. Les visites déjà vues ne se relancent pas seules (flag `seenTours`) ;
  rejouables via menu ⋯ → « Revoir le tutoriel & les visites guidées ».

## v0.15.1 — graphes d'évolution dans le dossier médical

- **Section « Évolution sur la période »** dans le dossier médical (entre synthèse des symptômes
  et transit) : deux graphes Recharts imprimables — sévérité des symptômes par jour (barres
  empilées léger/modéré/sévère) et transit (échelle de Bristol, zones constipation/diarrhée).
  Composant `components/MedicalRecordCharts.tsx`, animations coupées (imprimable dès l'ouverture),
  replis textuels si période trop courte ou rien à tracer.
- **Axe temporel honnête** : `fillDayRange(days)` (`lib/aggregates.ts`) comble les trous d'une
  liste de jours épars par des `emptyDay` (série contiguë du premier au dernier jour). Les jours
  vides autosauvés en base n'étirent pas l'axe (filtre `dayHasContent` avant remplissage) ; un
  trou de saisie reste visible comme un trou. Labels d'axe `dayMonthLabel` (« 9 juin »,
  `lib/dates.ts`) — le jour de semaine seul serait ambigu sur une longue période.
- **Code-splitting préservé** : `MedicalRecordSheet` passe en `React.lazy` et n'est monté qu'à
  l'ouverture (sinon le chunk recharts serait chargé au démarrage). Chunk dédié ~24 kB.
- **Impression** : règles `@media print` ajoutées pour les SVG (texte des axes en sombre, grille
  en clair) ; les remplissages sémantiques (barres, points) sont des attributs `fill`, préservés.
- Tests : `fillDayRange` (trous, fin de mois, listes vide/unitaire) + `dayMonthLabel` → 332 tests.
